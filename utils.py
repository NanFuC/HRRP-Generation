...
def smooth_crossfade(images, alpha):
    s = tf.shape(images)
    y = tf.reshape(images, [-1, s[0] // 2, 2, s[1], s[2]])
    y = tf.reduce_mean(y, axis= 2, keepdims=True)
    y = tf.tile(y, [1, 1, 2, 1, 1])
    y = tf.reshape(y, [-1, s[0], s[1], s[2]])
    images = lerp(images, y, alpha)
    return images

def preprocess_fit_train_image(images, res):
    images = tf.image.resize_images(images, size=[res, 1], method=tf.image.ResizeMethod.BILINEAR)
...
def load_data(dataset_name) :
    ...
    path = glob('./dataset/{}/*.*'.format(dataset_name + '/trainA'))
    print(path)
    data = []
    for i in range(len(path)):
        mat1 = sio.loadmat(path[i])
        data1 = np.reshape(mat1['data'],[-1,1024,1,1])
        
        if i ==0:
            data = data1
        else:
            data = np.concatenate([data1,data],axis=0)
    length_data = len(data[:,0,0,0])
    print(length_data)
    permutation = np.random.permutation(length_data)
