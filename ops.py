...
def conv(x, channels, kernel=3, stride=1, gain=np.sqrt(2), lrmul=1.0, sn=False, scope='conv_0'):#
    with tf.variable_scope(scope):
        weight_shape = [kernel, 1, x.get_shape().as_list()[-1], channels]#
        ...
        x = tf.nn.conv2d(input=x, filter=weight, strides=[1, stride, 1, 1], padding='SAME')

        return x
def conv2d(input_, output_dim, k_h=5, k_w=1, d_h=2, d_w=1, stddev=0.02, name="conv2d"):
...
#        w, sigma = spectral_normed_weight(w, num_iters=1, with_sigma=True, update_collection = "spectral_norm_update_ops")
...
def linear(input_, output_size, scope=None, stddev=0.02, bias_start=0.0, with_w=False):
    ...
    with tf.variable_scope(scope or "Linear"):
        ...
        '''spectral normalization'''
#        matrix, sigma = spectral_normed_weight(matrix, num_iters=1, with_sigma=True, update_collection = "spectral_norm_update_ops")
...
def compute_loss(real_images, real_logit, fake_logit):
    ...
    real_logit = tf.cast(real_logit,fake_logit.dtype)
    #r1_gamma.dtype = fake_logit.dtype
    ...
    gg = r1_penalty*(r1_gamma*0.5)
    gg= tf.cast(gg,fake_logit.dtype)
    
    d_loss = d_loss_gan + gg
...
def synthesis_const_block(res, w_broadcasted, n_f, sn=False):
    ...
    with tf.variable_scope('{:d}x{:d}'.format(res, res)):
        with tf.variable_scope('const_block'):
            x = tf.get_variable('Const', shape=[1, 4, 1, n_f], dtype=tf.float32, initializer=tf.initializers.ones())#
...
def _blur2d(x, f, normalize=True, flip=False, stride=1):
    ...
    f = np.tile(f, [3, 1, int(x.shape[-1]), 1])
...
def _upscale2d(x, factor=2, gain=1):
    ...
    s = x.shape # [bs, h, w, c]
    x = tf.reshape(x, [-1, s[1], 1, s[2], s[-1]])
    x = tf.tile(x, [1, 1,  factor, 1, 1])
    x = tf.reshape(x, [-1, s[1]*factor , s[2] , s[-1]])
    return x
